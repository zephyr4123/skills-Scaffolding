...
一套可复制的个人 Agent Skill 脚手架。它做三件事：

1. **收编**：把散落各处的 Claude Code skill 收进一个有版本管理的仓库，配上"每个是干啥的"总索引
2. **一键恢复**：任何机器上一条命令装齐全部 skill 和插件，幂等、只补缺、不重复下载
3. **一句话注入**：任何项目里对 Claude 说"装脚手架"，这个项目的每次会话就自动知道**什么场景用哪个 skill**（GUIDE.md）和**主人喜欢怎么干活**（HABITS.md）
...
装完即得：全部 21 个 skill 立即可用，且**每次会话自动注入** GUIDE（skill 路由）与 HABITS（协作习惯）——由插件的 SessionStart hook 完成，不需要跑脚本，也不需要说"装脚手架"。仓库有更新时插件自动同步。

- 工作/公司项目想关掉注入与 skill：`claude plugin disable zephyr-skills --scope project`
- 第三方插件（catalog 三件）不随本插件自动安装，按各自档案里的命令装
- **与 git 模式二选一**：已经用 install.sh + scaffold-init 的机器不要再装本插件，否则 GUIDE/HABITS 会注入两遍

### git 仓库模式（推荐给维护者 / 想改 skill 的人）
...
整个系统分三层，箭头都是符号链接或引用，**内容永远只有仓库里一份**：
...
先定位仓库根（下表 `<仓库>` 的取值，按顺序尝试）：① 默认 `~/coding/personal/skills-scaffolding`；② `readlink ~/.claude/skills/scaffold-init` 反查（链接目标的上三级就是仓库根）；③ 都没有则 `git clone https://github.com/zephyr4123/skills-Scaffolding.git ~/coding/personal/skills-scaffolding`。

| 用户意图 | 你该做的 |
|---|---|
| "装脚手架" / "注入 skill 指南" | 调用 `scaffold-init` skill；若本机没有该 skill，先按下一行恢复环境，再执行 `<仓库>/skills/general/scaffold-init/SKILL.md` 里的步骤 |
| 恢复/检查环境 | 先 `bash <仓库>/scripts/preflight.sh`（只读，退出码 0=完整 1=有缺口）；**仅当有缺口时**才 `bash <仓库>/scripts/install.sh` |
...
这些插件的本体由 Claude Code 插件市场管理（作者持续更新，仓库不存副本），`install.sh` 会按 `install.manifest` 自动安装：
...
| 事件 | 动作 |
|---|---|
| 收一个散装 skill | 副本放进 `skills/<领域>/` → README 索引加一行 → GUIDE.md 路由加一条 → plugin.json `skills` 数组加一条 → 跑 install.sh |
...
| 悟出新的经验习惯 | HABITS.md 加一行，完事（全部已注入项目自动继承） |
| 自己写新 skill | 从 `templates/skill-template/` 复制起步，写完按"收散装 skill"流程走 |
| 改了 GUIDE/HABITS/skill 正文 | commit + push 即可，所有机器 pull 后自动生效（符号链接的好处） |
...
1. 三处 clone URL 换成你的 fork：README「快速开始」、README「给 Claude Code 的操作手册」、`skills/general/scaffold-init/SKILL.md`；`.claude-plugin/` 里的市场名、插件名、owner 也换成你自己的
