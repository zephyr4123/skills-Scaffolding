...
│  GUIDE.md                 场景→skill 路由表（面向模型）      │
│  HABITS.md                协作习惯与经验（面向模型）         │
...
┌─ 项目层 ──────▼───────────────────────────────────────────┐
│  <项目>/.claude/skills-guide.md ──符号链接──▶ 仓库/GUIDE.md  │
│  <项目>/.claude/habits.md       ──符号链接──▶ 仓库/HABITS.md │
...
| 收编一个新 skill | 复制 skill 目录进 `skills/<领域>/`（去掉内嵌 `.git`，保留 LICENSE；没有合适领域可新建目录，脚本按 `skills/*/*/` 自动识别），然后：README 索引表加一行**并更新分组标题里的计数**、GUIDE.md 路由加一条，最后跑一次 install.sh 让链接生效 |
...
GUIDE.md          场景→skill 路由表，scaffold-init 注入项目用（面向模型）
HABITS.md         协作习惯与经验，scaffold-init 注入项目用（面向模型，活文档）
...
| 事件 | 动作 |
|---|---|
| 收一个散装 skill | 副本放进 `skills/<领域>/` → README 索引加一行 → GUIDE.md 路由加一条 → 跑 install.sh |
...
1. 三处 clone URL 换成你的 fork：README「快速开始」、README「给 Claude Code 的操作手册」、`skills/general/scaffold-init/SKILL.md`
