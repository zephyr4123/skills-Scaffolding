...
GUIDE.md          面向模型的场景路由指南（什么场景用哪个 skill），scaffold-init 注入用
HABITS.md         协作习惯与经验（commit 规则、做事方式、汇报风格），scaffold-init 注入用
...
- **启动新项目** → 在项目里对 Claude 说"装脚手架"（scaffold-init 先跑 preflight.sh 预检查：本机环境完整就零安装直接注入；缺啥补啥、绝不重复下载；全新机器会先自动 clone 本仓库。然后把 [GUIDE.md](GUIDE.md) 和 [HABITS.md](HABITS.md) 以 `@import` 注入项目 CLAUDE.md，此后每次会话自动知道什么场景用哪个 skill、以及主人的协作习惯；两个文件更新后全项目自动跟进）
- **悟出新的经验习惯** → 往 [HABITS.md](HABITS.md) 加一行，所有装过脚手架的项目自动继承
