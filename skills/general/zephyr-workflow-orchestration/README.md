...
本 skill = **可执行层**(怎么做的决策流 + 纪律)。深层第一性原理、六条承重原则、每种范式的完整 grounding = **参考层**,住在 Multica `above-the-wind` 方法论簇:hub **ATW-26(#8)** + 5 支 **ATW-28~32** + 配合锚 **ATW-27(#9)**。skill 记不下的深度去那里查——这正呼应 [zephyr-multica-read](../zephyr-multica-read/) 的主张:让沉淀**可读**,方法论才算闭环。
...
---
distill 自 above-the-wind 项目(用户点名);是个人方法论 **#7**(项目需求 → 可复用 skill 回流脚手架)的**第二个实例**(第一个 = zephyr-multica-read)。本 skill 是个人的,非 AE。
