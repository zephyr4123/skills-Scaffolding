...
8. **全景记忆读取移交 `zephyr-multica-read`**：单条任务的实时观测（runs/run-messages/comment list）归本 skill；要把 workspace 沉淀的记忆**批量读全**（冷启动 onboarding、取证溯源、按标签/时间/全文检索、成本分析），移交 `zephyr-multica-read` skill——它有只读白名单网关和 token 纪律，别用裸 CLI 硬读。
...
| agent 早干完在 in_review 等着，我半天才发现 | **Multica 不会把通知推进你的会话**——CLI 无 watch/follow/webhook 能力，`issue subscriber` 只进 Multica 自己的收件箱。托管出去 = 你不问就不知道 | 架一个轮询监控，把「记得回去查」变成「它来找我」；**注意要给 `todo` 无人认领和 `in_progress` 挂死各配一个哨兵**，只监听状态变化会让这两种静默失败和正常运行长得一样。做法见 `zephyr-background-watch` skill（Claude Code 专属） |
