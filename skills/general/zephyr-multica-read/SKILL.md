...
# zephyr-multica-read：让 agent 读全 Multica 的持久化记忆
...
7. **写操作移交 `zephyr-multica-collab`** — 读取过程中产生的任何写需求(发 issue、加评论、改状态、派活、rerun)都不归本 skill;移交 `zephyr-multica-collab` skill 执行,那里有派活协议和确认纪律。本 skill 的终点永远是"把记忆喂出去"。
